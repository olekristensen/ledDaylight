...
//                    shutter.absValue = cameraSettings.getValue("shutter", 1.0); // ms
                    ...
                    string fileName = ofToDataPath("CameraShutterRel-" + ofToString(serialNumber) + ".csv");

                    if(ofFile::doesFileExist(fileName){
                        printf( "Reading absolute exposure table from " + fileName + " ...\n" );
...
// TODO Check all this:
                        ...
                        for(unsigned int shutter_count = 1; shutter_count <= 4096; shutter_count ++)
                        {
                            ...
                            buff << ofToString(prop.absValue) << endl;
                        }

                        bool fileWritten = ofBufferToFile(fileName, dataBuffer);
                    }
       ...
       floatImage.allocate(camWidth,camHeight);
